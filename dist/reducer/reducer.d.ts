 ...
 * ```
 */
export declare const createReducer: <State, Payloads extends PayloadsTree>(initialState: State, handlers: ActionHandlers<State, Payloads>) => ((state: State | undefined, action: any) => State);
/**
 ...
 */
export declare const createHandlers: <State, Payloads extends PayloadsTree>() => <Handlers extends { [Type in Extract<keyof Payloads, string>]: Payloads[Type] extends undefined ? (state: State) => State : (state: State, action: ActionWithPayload<Type, Payloads[Type]>) => State; }>(handlers: Handlers) => Handlers;
