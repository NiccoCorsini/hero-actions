...
export type ActionHandlers<State, Payloads extends PayloadsTree> = {
    [Type in Extract<keyof Payloads, string>]: Payloads[Type] extends undefined ? (state: State, action: Action<Type>) => State : (state: State, action: ActionWithPayload<Type, Payloads[Type]>) => State;
};
